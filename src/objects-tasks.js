...
function removeProperties(obj, keys) {
  const dest = { ...obj };
  keys.forEach((key) => delete dest[key]);
  return dest;
}
...
function compareObjects(obj1, obj2) {
  ...
  return keys1.reduce((acc, key) => {
    return acc && keys2.includes(key) && obj1[key] === obj2[key];
  }, true);
}
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((elem) => {
    ...
    if (map.has(key)) {
      map.get(key).push(valueSelector(elem));
    } else {
...
const cssSelectorBuilder = {
  element(value) {
    return new Handler().element(value);
  },
  ...
  class(value) {
    return new Handler().class(value);
  },

  attr(value) {
    return new Handler().attr(value);
  },

  pseudoClass(value) {
    return new Handler().pseudoClass(value);
  },

  pseudoElement(value) {
    return new Handler().pseudoElement(value);
  },

  combine(selector1, combinator, selector2) {
    return new Handler().combine(
      selector1.stringify(),
      combinator,
      selector2.stringify()
    );
